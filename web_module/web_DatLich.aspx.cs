...
public partial class web_module_web_DatLich : System.Web.UI.Page
{
    ...
    {
        var getIDKhachHang = from kh in db.tb_Users where kh.user_Phone == Request.Cookies["UserNail"].Value || kh.user_Email == Request.Cookies["UserNail"].Value select kh;
        var getDataDatLich = from hdt in db.tb_HoaDons
                             ...
                             where hdt.khachhang_id == getIDKhachHang.FirstOrDefault().user_Id
                             && hdt.hoadon_hidden == "Tạo đặt lịch" && hdt.hoadon_loaithanhtoan == "Đặt lịch" && hdt.hoadon_tinhtrang == "Order"
                             select new
                             {
                                 ...
                                 thanhtien = hdctt.hoadonchitiet_soluong * hdctt.hoadonchitiet_donggia,
                                 hdctt.hoadonchitiet_hinhanhchuyenkhoan,
        ...
        if (Session["vourcher"] != null)
        {
            ...
            var checkMaVach = (from mv in db.tb_MaGiamGiaCuaKhachHangs
                               ...
                               where mv.khachhang_id == getIDKhachHang.FirstOrDefault().user_Id
    ...
    protected void btnXoaSanPham_ServerClick(object sender, EventArgs e)
    {
        ...
        var XoaId = (from hdt in db.tb_HoaDons
                     join hdctt in db.tb_HoaDonChiTiets on hdt.hoadon_id equals hdctt.hoadon_id
                     where hdt.khachhang_id == getIDKhachHang.FirstOrDefault().user_Id && hdt.hoadon_hidden == "Tạo đặt lịch" && hdt.hoadon_loaithanhtoan == "Đặt lịch"
    ...
    {
        if (txtNgay.Text == "" || txtThoiGian.Text == "")
        {
        ...
        else
        {
            ...
            tb_HoaDon updateTinhTrang = (from hdt in db.tb_HoaDons
                                         where hdt.khachhang_id == getIDKhachHang.FirstOrDefault().user_Id
                                           && hdt.hoadon_tinhtrang == "Order" && hdt.hoadon_loaithanhtoan == "Đặt Lịch"
                                         select hdt).SingleOrDefault();
            updateTinhTrang.hoadon_createdate = Convert.ToDateTime(txtNgay.Text);
            updateTinhTrang.hoadon_giovao = txtThoiGian.Text;
            ...
            updateTinhTrang.hoadon_tongtien = Convert.ToString(tien);
            db.SubmitChanges();
    ...
    private bool SendMail()
    {
        try
        {
            var fromAddress = "tinnhankhachhang@gmail.com";//  tinnhankhachhang@gmail.com
                                                           // pass : abc123#!
            var toAddress = "danbichlai21@gmail.com"; // 
