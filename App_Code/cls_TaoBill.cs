   ...
   public bool tao_Bill( int idsanpham, string phone)
    {
        ...
        var getIDKhachHang = from kh in db.tb_Users where kh.user_Phone == phone select kh;
        // Kiểm tra khách hàng có tạo đặt lịch trong bảng tạm chưa
        var checkTaoLichTam = from dht in db.tb_HoaDons where /*dht.hoadon_hidden == "Tạo đặt lịch" &&*/ dht.hoadon_tinhtrang=="Order" && dht.khachhang_id == getIDKhachHang.FirstOrDefault().user_Id select dht;
        if (checkTaoLichTam.Count() > 0)
        {
            ...
            var checkSanPhamTam = from sp in db.tb_HoaDonChiTiets
                                  ...
                                  where sp.sanpham_id == idsanpham && hdt.hoadon_hidden == "Tạo đặt lịch" && hdt.hoadon_tinhtrang=="Order" && hdt.khachhang_id == getIDKhachHang.FirstOrDefault().user_Id
                                  select sp;
            ...
            else
            {
               ...
               // insertCT.hoadon_id = checkTaoLichTam.FirstOrDefault().hoadon_id;
                ...
                insertCT.hoadonchitiet_donggia = Convert.ToInt32(checkSanPham.FirstOrDefault().sanpham_gia);
                insertCT.hoadonchitiet_soluong = 1;
                insertCT.hoadonchitiet_thanhtien =Convert.ToString( insertCT.hoadonchitiet_donggia * insertCT.hoadonchitiet_soluong);
        ...
        else
        {
          ...
          //  insert.hoadon_code = hamTuDongTang();
            insert.khachhang_id = getIDKhachHang.FirstOrDefault().user_Id;
            insert.hoadon_sdt = getIDKhachHang.FirstOrDefault().user_Phone;
           // insert.hoadon_TenKhachHang = getIDKhachHang.FirstOrDefault().khachhang_name;
            insert.hoadon_hidden = "Tạo đặt lịch";
             insert.hoadon_loaithanhtoan = "Đặt lịch";
